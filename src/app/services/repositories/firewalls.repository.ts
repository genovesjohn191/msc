...
export class FirewallsRepository extends McsRepositoryBase<McsFirewall> {
  ...
  public findFirewallPolicies(
    ...
    return this._firewallsApiService.getFirewallPolicies(
      activeFirewall.id,
      {
        page: page.pageIndex,
  ...
  protected getAllRecords(
    ...
    return this._firewallsApiService.getFirewalls({
      page: pageIndex,
      perPage: pageSize,
  ...
  protected getRecordById(recordId: string): Observable<McsApiSuccessResponse<McsFirewall>> {
    return this._firewallsApiService.getFirewall(recordId);
  }
}
